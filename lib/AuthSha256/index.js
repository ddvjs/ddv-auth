...
Object.assign(AuthSha256Prototype, {
  ...
  setSignTimeString: function setSignTimeString (signTimeString) {
    if (typeof signTimeString === 'number') {
      signTimeString = time.gmdate('Y-m-d H:i:s', signTimeString)
    } else if (signTimeString instanceof Date) {
      signTimeString = time.gmdate('Y-m-d H:i:s', parseInt(signTimeString.getTime() / 1000))
    }
  ...
  getSigningKey: function getSigningKey (authString) {
    ...
    // 生成加密key
    var signingKey = hash_hmac('sha256', authString, this.accessKey)
    return signingKey
  },
  ...
  getAuthArray: function getAuthArray () {
    ...
    // 服务端模拟客户端算出的签名信息
    var signMsg = hash_hmac('sha256', canonicalRequest, signingKey)
    // 组成最终签名串
