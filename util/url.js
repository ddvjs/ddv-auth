  ...
  /**
    ...
    for (i = 0; i < t.length; i++) {
      ...
      for (i = 0; i < keys.length; i++) {
          ...
          urli['path'] = rigthTemp?leftTemp:(leftTemp+'/'+rigthTemp)
			...
			if (parts && parts['query'] && (flags & url.HTTP_URL_JOIN_QUERY)) {
				if (isset(urli['query'])) {
					...
					urli['query'] = url.buildQuery(util.extend({},urliQuery,urliQuery));
				} else {
		...
		if (isset(urli['path']) && urli['path'] !== '' && substr(urli['path'], 0, 1) !== '/') {
			urli['path'] = '/' . urli['path'];
		}
    ...
    for (i = 0; i < keys.length; i++) {
		...
		if (urli['scheme']) {
			parsed_string += urli['scheme'] + '://';
		}

		if (urli['user']) {
			parsed_string += urli['user'];

			if (urli['pass']) {
				parsed_string += ':' + urli['pass'];
			}
		...
		if (urli['host']) {
			parsed_string += urli['host'];
		}

		if (urli['port']) {
			parsed_string += ':' + urli['port'];
		}

		if (urli['path']) {
			parsed_string += urli['path'];
		}

		if (urli['query']) {
			parsed_string += '?' + urli['query'];
		}

		if (urli['fragment']) {
			parsed_string += '#' + urli['fragment'];
		}
